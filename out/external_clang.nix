{ cc_defaults, cc_library_headers, cc_library_shared, clang_tblgen, genrule, llvm_tblgen }:
let

clang-defaults = cc_defaults {
    name = "clang-defaults";
    defaults = [
        "llvm-defaults"
    ];
...
clang-headers = cc_library_headers {
    name = "clang-headers";
...
libclang_android = cc_library_shared {
