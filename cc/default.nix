{ pkgs, lib, bpPkgs, packageSrc, sourceDir, resolveSrcs, genrule }:
...
#      ldflags = [ "-nostdinc++" ];
    ...
    outSuffix =
      ...
      else if suffix == "yy" || suffix == "ll" then "cc"
      else "o";
  in {
    out = [ "${root}.o" ]
      ++ optionals (hasSuffix ".y" src || hasSuffix ".yy" src) [ "${root}.h" ]
      ++ optionals (hasSuffix ".y" src || hasSuffix ".yy" src || hasSuffix ".l" src || hasSuffix ".ll" src) [ "${root}.${outSuffix}" ];
