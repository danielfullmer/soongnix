...
let
    ...
    let f = attrPath:
      zipAttrsWith (n: values:
        if tail values == []
          ...
          then concatLists values # No longer deduping here. Fixes libc_freebsd
  ...
  linuxCflags = [
    ...
    # TODO(soongnix): jemalloc fails to build with bionic when this is enabled
    #"-U_FORTIFY_SOURCE"
    #"-D_FORTIFY_SOURCE=2"
  ...
  commonGlobalIncludes = map (d: "-I${sourceDir d}") [
    "system/core/include"
    ...
    "framework/native/opengl/include"
    "frameworks/av/include"
  ];

  kernelArch = "x86";
  bionicHeaders = [
    "-isystem ${sourceDir "bionic/libc"}/include"
    "-isystem ${sourceDir "bionic/libc"}/kernel/uapi"
    "-isystem ${sourceDir "bionic/libc"}/kernel/uapi/asm-${kernelArch}"
    "-isystem ${sourceDir "bionic/libc"}/kernel/android/scsi"
    "-isystem ${sourceDir "bionic/libc"}/kernel/android/uapi"
  ];
  ...
  ld = { rsp, out, ldCmd ? "clang++", crtBegin ? "", libFlags ? [], crtEnd ? "", ldFlags ? [] }:
      "${clang}/bin/${ldCmd} @${rsp} ${toString libFlags} -o ${out} ${toString ldFlags} ${hostLdLibs} -Wl,--start-group -lgcc -lgcc_eh -lc -Wl,--end-group";

  cc = { src, out, ccCmd, cflags ? [] }:
    "${clang}/bin/${ccCmd} -c ${toString cflags} -o ${out} ${src}";
...
#      ldflags = [ "-nostdinc++" ];
  ...
  in {
    ...
    cmd = if suffix == "asm" then
      "${pkgs.yasm}/bin/yasm ${asflags} ${includeFlags} -D__ASSEMBLY__ -o ${root}.o ${absSrc}"
    else if (suffix == "y" || suffix == "yy") then
      "BISON_PKGDATADIR=${sourceDir "prebuilts/build-tools/common/bison"} ${bison}/bin/bison -d ${yaccflags} --defines=${root}.h -o ${root}.${outSuffix} ${absSrc}"
      ...
      "${bpPkgs.aidl}/bin/aidl ${aidlflags} ${absSrc} ${root}.${outSuffix}"
      + " && " + (mkObjectFile args "./${root}.${outSuffix}").cmd
  ...
  '' else ''
    ${clang}/bin/clang -o $out ${map lookupPkg objs}
  '')));
